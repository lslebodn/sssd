                       ...
                       tevent_timer_handler_t handler)
{
    ...
    te = tevent_add_timer(ev, nss_ctx, tv, handler, enum_ctx);
    if (te == NULL) {
...
struct nss_setent_internal_state {
    ...
    struct nss_ctx *nss_ctx;
    struct nss_enum_ctx *enum_ctx;
    tevent_timer_handler_t timeout_handler;
...
static void nss_setent_internal_done(struct tevent_req *subreq)
{
    ...
    /* Expire the result object after enum cache timeout is reached. */
    tret = nss_setent_set_timeout(state->ev, state->nss_ctx, state->enum_ctx,
